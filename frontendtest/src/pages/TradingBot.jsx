...
const STRATEGIES = ['default', 'crossover', 'momentum'];
    ...
    <div className="flex justify-center items-center min-h-screen bg-black text-white">
      <div className="w-full max-w-3xl p-6 border border-gray-700 rounded-lg">
        ...
        <div className="flex gap-2 mb-4">
          <button className="bg-green-500 px-4 py-2 rounded" onClick={handleStart} disabled={loading}>{loading ? 'Starting...' : 'Start Bot'}</button>
          <button className="bg-red-500 px-4 py-2 rounded" onClick={handleStop} disabled={loading}>{loading ? 'Stopping...' : 'Stop Bot'}</button>
          <button className="bg-blue-500 px-4 py-2 rounded" onClick={handleStatus} disabled={loading}>{loading ? 'Checking...' : 'Check Status'}</button>
        </div>

        {error && <div className="text-red-500 mb-4">{error}</div>}
        {status && <div className="mb-4">Status: {status}</div>}
        {logs.length > 0 && (
